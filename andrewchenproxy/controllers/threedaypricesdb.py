...
class threedaypricesdb:

  def GET(self):

    prices = memcache.get('threedaypricesdb')
    ...
    sqls = ["""SELECT destination_airport, there_date, back_date, check_date, price FROM 3dayweekends""", """SELECT * FROM 3dayweekends where latest = 1 and there_date > '%s'""" % (day)]
